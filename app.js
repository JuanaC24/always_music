...
const runQuery = async (text, values, action) => {
  const client = await pool.connect();
  try {
    const res = await client.query(text, values);
    console.log(`${action}:`, res.rows.length ? res.rows[0] : 'No results found');
  } catch (err) {
...
const commands = {
  add: async ({ nombre, rut, curso, nivel }) => {
    ...
    await runQuery(text, values, 'Estudiante agregado');
  },
  get: async ({ rut }) => {
    const text = 'SELECT * FROM estudiantes WHERE rut = $1';
    const values = [rut];
    await runQuery(text, values, 'Estudiante');
  },
  getAll: async () => {
  ...
  update: async ({ nombre, rut, curso, nivel }) => {
    ...
    await runQuery(text, values, 'Estudiante actualizado');
  },
  delete: async ({ rut }) => {
    const text = 'DELETE FROM estudiantes WHERE rut = $1 RETURNING *';
    const values = [rut];
    await runQuery(text, values, 'Estudiante eliminado');
  }
};
